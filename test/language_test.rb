...
class TestLexicalHash < Test::Unit::TestCase
  ...
  def test_params
    ...
    assert_raise(SystemExit) { Lingo::LexicalHash.new( 'nonsens' ) }
  end

  #  TODO: Crypt testen...

  def test_cache
    ds = Lingo::LexicalHash.new( 'sys-dic' )
  ...
  def test_report
    ds = Lingo::LexicalHash.new( 'tst-syn' )
  ...
  def test_auto_create
    txt_file = File.join(File.dirname(__FILE__), '..', 'dict', @database_config['tst-sgw']['name'])
    sto_file = txt_file.gsub(/\/de\//, '/de/store/').gsub(/\.txt/, '.pag')

    ds = Lingo::LexicalHash.new( 'tst-sgw' )
  ...
  def test_keyvalue
    ds = Lingo::LexicalHash.new( 'sys-mul' )
  ...
  def test_multivalue
    ds = Lingo::LexicalHash.new( 'sys-syn' )
...
class TestDictionary < Test::Unit::TestCase
  ...
  def test_params
    ...
    assert_raise(RuntimeError) { Lingo::Dictionary.new({'source'=>['sys-dic']}, nil) }
    #  Keine Parameter angegeben
    assert_raise(RuntimeError) { Lingo::Dictionary.new(nil, @dictionary_config) }
    #  Falsche Parameter angegeben (Pflichtparameter ohne Defaultwert)
    assert_raise(RuntimeError) { Lingo::Dictionary.new({'course'=>['sys-dic']}, @dictionary_config) }
  end


  def test_cache
    dic = Lingo::Dictionary.new({'source'=>['sys-dic']}, @dictionary_config)
  ...
  def test_select_two_sources_mode_first
    dic = Lingo::Dictionary.new({'source'=>['sys-dic', 'tst-dic'], 'mode'=>'first'}, @dictionary_config)
  ...
  def test_select_two_sources_mode_first_flipped
    dic = Lingo::Dictionary.new({'source'=>['tst-dic','sys-dic'], 'mode'=>'first'}, @dictionary_config)
    #  in keiner Quelle vorhanden
    assert_equal([], dic.select('hasennasen'))
    #  nur in erster Quelle vorhanden
    assert_equal([lx('knaller|s')], dic.select('knaller'))
  ...
  def test_select_two_sources_mode_all
    dic = Lingo::Dictionary.new({'source'=>['sys-dic','tst-dic'], 'mode'=>'all'}, @dictionary_config)
    #  in keiner Quelle vorhanden
    assert_equal([], dic.select('hasennasen'))
    #  nur in erster Quelle vorhanden
    assert_equal([lx('knaller|s')], dic.select('knaller'))
  ...
  def test_select_two_sources_mode_default
    dic = Lingo::Dictionary.new({'source'=>['sys-dic','tst-dic']}, @dictionary_config)
    #  in keiner Quelle vorhanden
    assert_equal([], dic.select('hasennasen'))
    #  nur in erster Quelle vorhanden
    assert_equal([lx('knaller|s')], dic.select('knaller'))
...
class TestGrammar < Test::Unit::TestCase
  ...
  def test_cache
    gra = Lingo::Grammar.new({'source'=>['sys-dic']}, @dictionary_config)
    assert_equal(
  ...
  def test_max_parts
    ...
    assert_equal(Lingo::Word.new('baumsbaumsbaumsbaumsbaumsbaum', Lingo::Const::WA_UNKNOWN), gra.find_compositum('baumsbaumsbaumsbaumsbaumsbaum'))
    gra.close
  end
