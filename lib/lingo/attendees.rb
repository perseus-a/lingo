...
class Lingo::Attendee
  ...
  def listen(obj)
    unless obj.is_a?(Lingo::AgendaItem)
    ...
    if param.nil?
      ...
      case obj
        ...
        when STR_CMD_ERR  then printf "%s\n=   %s: %s\n%s\n", '='*61, @config['name'], param, '='*61;  exit( 1 )
          ...
          talk(Lingo::AgendaItem.new(obj, param))
      end
    end
  end
...
class Lingo::BufferedAttendee < Lingo::Attendee
  ...
  def deferred_insert(pos, obj)
    @deferred_inserts << Lingo::BufferInsert.new(pos, obj)
  end
