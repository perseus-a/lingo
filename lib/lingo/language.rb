...
=end
class Lingo::LexicalHash

  include Lingo::Const
  include Lingo::Cachable
  include Lingo::Reportable
  ...
  def [](ikey)
    ...
    record = record.collect do |str|
      ...
      when /^#(.)$/            then Lingo::Lexical.new(key, $1)
      #  Angabe Ergebniswort und Wortklasse
      when /^([^#]+?)\s*#(.)$/ then Lingo::Lexical.new($1, $2)
      #  Angabe Ergebniswort ohne Wortklasse
      when /^([^#]+)$/         then Lingo::Lexical.new($1, @wordclass )
      else  str
      end
...
class Lingo::Dictionary
  ...
  def find_word(string)
    ...
    word = Lingo::Word.new(string, WA_UNKNOWN)
  ...
  def select(string)
    ...
    # TODO: anders lösen
    lex = lexicals.collect { |i| i.is_a?(Lingo::Lexical) ? i : nil }.compact.sort.uniqual
  ...
  def suffix_lexicals(string)
    ...
    @suffixes.each { |suf|
      ...
      if string =~ regex
        newform = $`+((ext=="*")?'':ext)+$'
        lexicals << Lingo::Lexical.new(newform, type)
      end
    }
    lexicals
  end
...
class Lingo::Grammar
  ...
  def initialize(config, dictionary_config)
    ...
    @dictionary = Lingo::Dictionary.new(config, dictionary_config)
  ...
  def find_compositum(string, level=1, has_tail=false)
    ...
    #   Ergebnis vorbelegen
    comp = Lingo::Word.new(string, WA_UNKNOWN)
    ...
    if level==1
      ...
      if lexis.size > 0 && is_valid?( string, stats, lexis, seqs )
        ...
        comp.lexicals = lexis.collect do |lex|
          (lex.attr==LA_KOMPOSITUM) ? lex : Lingo::Lexical.new(lex.form, lex.attr+@append_wc)
        end
      end
  ...
  def test_compositum(front_string, infix, back_string, level, has_tail)
    ...
    if back_lexicals.empty? && infix=='-'
      back_lexicals = [Lingo::Lexical.new(back_string, LA_TAKEITASIS)]
      back_form = back_string
      seqs[1] = back_lexicals.sort[0].attr
    end
    ...
    if front_lexicals.empty? && infix=='-'
      front_lexicals = [Lingo::Lexical.new(front_string, LA_TAKEITASIS)]
      seqs[0] = front_lexicals.sort[0].attr
      front_form = front_string
    end
    ...
    lexis << Lingo::Lexical.new(front_form + infix + back_form, LA_KOMPOSITUM)
